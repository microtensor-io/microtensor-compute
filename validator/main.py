...
from protocol.session import release_message
from validator import __version__
from validator.config import Settings
from validator.config import settings as load_settings
from validator.identity import Identity, IdentityError, load_keypair
from validator.pool import PoolClient, ServerError
from validator.schedule.deep import DeepPass
from validator.schedule.express import ExpressLane
from validator.scoring.demand_weight import DemandWeights
from validator.scoring.reliability import UptimeLedger
from validator.scoring.weights import Chain, compute_scores
from validator.session.client import AgentClient
...
class Validator:
    def __init__(self, settings: Settings) -> None:
        ...
        self.identity = Identity(load_keypair(settings))
    ...
    async def run(self) -> int:
        ...
        try:
            await asyncio.gather(self.express.loop(), self.deep.loop(), self.weights_loop())
        finally:
            self.ledger.save()
            await self.close()
        return 0
